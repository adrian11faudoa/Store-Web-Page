...
// Returns { exists: bool, name? } — used by frontend to decide login vs register flow
router.post('/check-identifier', authLimiter, async (req, res, next) => {
  try {
    ...
    res.json({ exists: true, name: user.name || null })
  } catch (err) { next(err) }
})

// ── POST /api/auth/register ───────────────────────────────────────────────────
// Creates an UNVERIFIED account and sends a verification email or WhatsApp.
router.post('/register', authLimiter, async (req, res, next) => {
  try {
    const { email: rawIdentifier, password, name } = req.body
    ...
    const existing = await pool.query('SELECT id, email_verified FROM users WHERE email=$1', [identifier])

    if (existing.rows.length) {
      const u = existing.rows[0]
      if (u.email_verified) return res.status(409).json({ error: 'Account already registered' })
      // Resend code
      ...
      if (isPhone) {
        await sendWhatsAppVerification({ to: identifier, name: name || null, code })
      } else {
        await sendVerificationEmail({ to: identifier, name: name || null, code })
      }
    ...
    if (isPhone) {
      await sendWhatsAppVerification({ to: identifier, name: user.name, code })
    } else {
      await sendVerificationEmail({ to: user.email, name: user.name, code })
    }

    res.status(201).json({ pending: true, message: 'Account created — please verify' })
  } catch (err) { next(err) }
})
...
router.post('/resend-verification', authLimiter, async (req, res, next) => {
  try {
    ...
    if (!rawIdentifier) return res.status(400).json({ error: 'Identifier required' })
    const identifier = rawIdentifier.toLowerCase()
    const isPhone    = /^\+\d{7,}$/.test(identifier)

    const userRes = await pool.query('SELECT id,name,email_verified FROM users WHERE email=$1', [identifier])
    if (!userRes.rows.length || userRes.rows[0].email_verified) return res.json({ ok: true })
    ...
    if (isPhone) {
      ...
      await sendVerificationEmail({ to: identifier, name: user.name, code })
    }
    res.json({ ok: true })
  } catch (err) { next(err) }
})

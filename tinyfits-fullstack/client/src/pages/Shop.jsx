...
export default function Shop() {
  ...
  const { products, total, totalPages, loading, error, filters, update, setPage } = useProducts({
  ...
  useEffect(() => {
    catsApi.list().then(data => setCats([{slug:'all',label:'All',product_count:total}, ...data]))
    ...
    <div className="shop-layout">
      ...
      <aside className="sidebar">
        ...
        <button
          ...
          onClick={() => update({ageGroup:'all'})}>All ages</button>
        {ageGroups.map(a => (
          <button key={a.slug}
            className={`sidebar-option${filters.ageGroup === a.slug ? ' active' : ''}`}
            onClick={() => update({ageGroup: a.slug})}>
            {a.label} <span className="sidebar-count">{a.product_count}</span>
          </button>
        ))}
      ...
      <main className="shop-main">
        <div className="shop-toolbar">
          <div className="search-box">
            ...
            <input type="text" placeholder="Search 900+ products…"
              value={filters.q}
              onChange={e => update({q: e.target.value})} />
          </div>
          <span className="result-count"><strong>{total}</strong> items</span>
        ...
        {error && <p style={{color:'var(--color-accent)',padding:'1rem'}}>⚠ {error}</p>}
        ...
        {totalPages > 1 && (
          <div className="pagination">
            ...
            {totalPages > 7 && <span className="page-btn" style={{border:'none'}}>…{totalPages}</span>}
            <button className="page-btn" disabled={filters.page >= totalPages}
